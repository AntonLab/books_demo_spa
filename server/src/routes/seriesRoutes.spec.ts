...
// No request in this file reaches /api/users, but createApp requires the
// dependency — a stub that throws keeps that assumption honest.
function createUnusedUserRepository(): UserRepository {
  const unreachable = (): never => {
    throw new Error('the user repository must not be used by these tests');
  };
...
async function withServer(fn: (base: string) => Promise<void>): Promise<void> {
  const app = createApp({
    userRepository: createUnusedUserRepository(),
